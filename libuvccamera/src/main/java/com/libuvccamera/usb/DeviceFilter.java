...
/*		Log.i(TAG, String.format("vendorId=0x%04x,productId=0x%04x,class=0x%02x,subclass=0x%02x,protocol=0x%02x",
	 ...
	 * @return
	 */
	private static final String getAttributeString(final Context context, final XmlPullParser parser, final String namespace, final String name, @NonNull final String defaultValue) {
        ...
        while (eventType != XmlPullParser.END_DOCUMENT) {
        	...
        	if (!TextUtils.isEmpty(tag) && (tag.equalsIgnoreCase("usb-device"))) {
        		if (eventType == XmlPullParser.START_TAG) {
        			...
        			manufacturerName = getAttributeString(context, parser, null, "manufacturer-name", "");
        			if (TextUtils.isEmpty(manufacturerName))
        				manufacturerName = getAttributeString(context, parser, null, "manufacture", "");
        			productName = getAttributeString(context, parser, null, "product-name", "");
        			if (TextUtils.isEmpty(productName))
        				productName = getAttributeString(context, parser, null, "product", "");
        			serialNumber = getAttributeString(context, parser, null, "serial-number", "");
        			if (TextUtils.isEmpty(serialNumber))
            			serialNumber = getAttributeString(context, parser, null, "serial", "");
