...
import com.libuvccamera.opengl.renderer.RendererHolderCallback;
...
//				mUVCCamera.setFrameCallback(mIFrameCallback, UVCCamera.PIXEL_FORMAT_YUV);
     ...
     */
    @Override
    public void takePicture(@FloatRange(from = 1) float zoom, ImageCapture.OutputFileOptions options, ImageCapture.OnImageCaptureCallback callback) {
        if (isCameraOpened() && mImageCapture != null) {
            mImageCapture.takePicture(zoom , options, callback);
        } else {
